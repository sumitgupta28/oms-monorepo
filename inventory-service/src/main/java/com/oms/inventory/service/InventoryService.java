...
public class InventoryService {
    ...
    public void reserveStock(UUID orderId, List<ReserveItem> items) {
        ...
        for (ReserveItem item : items) {
            ...
            if (!inv.canReserve(item.quantity())) {
                publishInsufficientEvent(orderId, item.productId(), item.quantity(), inv.getAvailableQty());
                return;
            }
            inv.reserve(item.quantity());
            inventoryRepo.save(inv);
            movementRepo.save(StockMovement.builder().productId(item.productId())
                .movementType(StockMovement.MovementType.RESERVE).delta(-item.quantity()).orderId(orderId).build());
            reserved.add(item.productId());
            if (inv.getAvailableQty() <= lowStockThreshold) {
                kafka.send("oms.inventory.low-stock",
                    "{\"productId\":\""+item.productId()+"\",\"availableQty\":"+inv.getAvailableQty()+"}");
            }
        }
        kafka.send("oms.inventory.reserved",
            "{\"eventType\":\"INVENTORY_RESERVED\",\"orderId\":\""+orderId+"\",\"productIds\":"+
            reserved.stream().collect(java.util.stream.Collectors.joining("\",\"","[\"","\"]"))+"}");
    ...
    public void releaseStock(UUID orderId, List<ReserveItem> items) {
        for (ReserveItem item : items) {
            inventoryRepo.findByIdWithLock(item.productId()).ifPresent(inv -> {
                ...
                inventoryRepo.save(inv);
                movementRepo.save(StockMovement.builder().productId(item.productId())
                    .movementType(StockMovement.MovementType.RELEASE).delta(item.quantity()).orderId(orderId).build());
            });
        }
    ...
    public InventoryResponse adjustStock(String productId, int delta) {
        ...
        inv.addStock(delta);
        movementRepo.save(StockMovement.builder().productId(productId)
            .movementType(StockMovement.MovementType.RESTOCK).delta(delta).build());
        return InventoryResponse.from(inventoryRepo.save(inv));
    }
    ...
    private void publishInsufficientEvent(UUID orderId, String productId, int requested, int available) {
        kafka.send("oms.inventory.insufficient",
            "{\"eventType\":\"INVENTORY_INSUFFICIENT\",\"orderId\":\""+orderId+
            "\",\"productId\":\""+productId+"\",\"requested\":"+requested+",\"available\":"+available+"}");
        log.warn("Insufficient stock for order {} product {}", orderId, productId);
    }
